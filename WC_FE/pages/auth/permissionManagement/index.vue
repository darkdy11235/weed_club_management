  ...
  <NuxtLayout name="custom">
    <section class="flex items-center justify-center h-screen bg-background">
      <div class="w-full max-w-screen-sm">
        <div
          class="flex items-center justify-center py-5 rounded-lg bg-surface"
          style="height: 400px"
        >
          <div class="p-6 space-y-4">
            <form class="space-y-4 table-auto" action="#">
              ...
              <tbody>
                <tr
                  v-for="(permission, i) in permissionData"
                ...
                >
                  ...
                  <td class="px-2 py-2">
                    <input
                      :checked="isPermissionChecked(permission.id_permission)"
...
const getPermission = async () => {
  try {
    const response = await axios.get(
      `${URL_BE}/api/v1/author/get_all_permission`
    )
...
const changeAdmin = async () => {
  try {
    const response = await axios.get(
      `${URL_BE}/api/v1/author/${selectedRoleId.value}/get_permissions/`
    )
  ...
  try {
    for (const permission of permissionData.value) {
      if (rolePermission.value[permission.id_permission]) {
        ...
        allPermission.value.splice(
          allPermission.value.indexOf(permission.id_permission),
          1
        )
      }
    }
